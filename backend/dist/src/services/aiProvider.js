...
exports.ProviderErrorCode = void 0;
...
(function (ProviderErrorCode) {
    ProviderErrorCode["INVALID_KEY"] = "INVALID_KEY";
    ProviderErrorCode["QUOTA_EXCEEDED"] = "QUOTA_EXCEEDED";
    ProviderErrorCode["RATE_LIMIT"] = "RATE_LIMIT";
    ProviderErrorCode["TIMEOUT"] = "TIMEOUT";
    ProviderErrorCode["NETWORK_ERROR"] = "NETWORK_ERROR";
    ProviderErrorCode["UNSUPPORTED_MODEL"] = "UNSUPPORTED_MODEL";
    ProviderErrorCode["INVALID_REQUEST"] = "INVALID_REQUEST";
    ProviderErrorCode["CONCURRENT_REQUEST"] = "CONCURRENT_REQUEST";
    ProviderErrorCode["UNKNOWN_ERROR"] = "UNKNOWN_ERROR";
})(ProviderErrorCode || (exports.ProviderErrorCode = ProviderErrorCode = {}));
...
function validateApiKeyRuntime(key, provider) {
...
function detectErrorCode(error, provider) {
...
function getUserFriendlyError(errorCode, provider) {
    switch (errorCode) {
...
// Gemini Provider
async function callGeminiProvider(request) {
...
// Groq Provider
async function callGroqProvider(request) {
    const { message, systemPrompt, history, apiKey, provider, timeout = 30000 } = request;
    // Runtime key validation
    if (!validateApiKeyRuntime(apiKey, provider)) {
        return {
...
// Cerebras Provider
async function callCerebrasProvider(request) {
    const { message, systemPrompt, history, apiKey, provider, timeout = 30000 } = request;
    // Runtime key validation
    if (!validateApiKeyRuntime(apiKey, provider)) {
        return {
    ...
    try {
        ...
        const response = await Promise.race([
            axios_1.default.post('https://api.cerebras.ai/v1/chat/completions', {
                messages,
                model: 'llama3.1-8b',
                temperature: 0.7,
                max_tokens: 2048,
...
// OpenRouter Provider
async function callOpenRouterProvider(request) {
    const { message, systemPrompt, history, apiKey, provider, timeout = 30000 } = request;
    // Runtime key validation
    if (!validateApiKeyRuntime(apiKey, provider)) {
        return {
    ...
    try {
        ...
        messages.push({ role: 'user', content: message });
        const response = await Promise.race([
            axios_1.default.post('https://openrouter.ai/api/v1/chat/completions', {
                messages,
...
async function callAIProvider(request, availableKeys) {
    // Validate all keys at runtime
    const validKeys = availableKeys.filter(({ key, provider }) => validateApiKeyRuntime(key, provider));
    ...
    const providerOrder = ['gemini', 'groq', 'cerebras', 'openrouter'];
