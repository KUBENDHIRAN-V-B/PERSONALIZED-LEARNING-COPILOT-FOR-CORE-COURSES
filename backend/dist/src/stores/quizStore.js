...
exports.getQuizHistory = exports.finalizeQuiz = exports.submitAnswerAndAdvance = exports.getQuizSession = exports.createQuizSession = exports.toPublicQuestion = void 0;
const analyticsStore_1 = require("./analyticsStore");
const sessionsById = new Map();
const historyByUser = new Map();
const difficultyOrder = ['easy', 'medium', 'hard'];
const clampDifficulty = (idx) => difficultyOrder[Math.max(0, Math.min(difficultyOrder.length - 1, idx))];
...
const getQuestionsFor = (topicKey, difficulty) => questionBank.filter(q => q.topicKey === topicKey && q.difficulty === difficulty);
const pickNextQuestion = (session) => {
...
const toPublicQuestion = (q, topic) => ({
    id: q.id,
    topic,
...
const createQuizSession = (params) => {
    const id = `${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const topicKey = (0, analyticsStore_1.normalizeTopicKey)(params.topic);
    const session = {
        id,
    ...
    return { session, firstQuestion: (0, exports.toPublicQuestion)(next, params.topic) };
};
exports.createQuizSession = createQuizSession;
const getQuizSession = (sessionId) => sessionsById.get(sessionId) || null;
exports.getQuizSession = getQuizSession;
const submitAnswerAndAdvance = (params) => {
    const session = sessionsById.get(params.sessionId);
    if (!session)
        throw new Error('Quiz session not found');
    if (session.userId !== params.userId)
        throw new Error('Unauthorized for this quiz session');
    if (session.completed)
        throw new Error('Quiz session already completed');
    const q = questionBank.find(x => x.id === params.questionId);
    if (!q)
        throw new Error('Question not found');
    if (q.topicKey !== session.topicKey)
        throw new Error('Question does not match session topic');
    ...
    return {
        ...
        correctIndex: q.correctIndex,
        explanation: q.explanation,
        nextQuestion: (0, exports.toPublicQuestion)(next, session.topic),
        progress: { current: session.currentIndex, total: session.targetCount },
        updatedDifficulty: session.currentDifficulty,
    };
};
exports.submitAnswerAndAdvance = submitAnswerAndAdvance;
const finalizeQuiz = (params) => {
    const session = sessionsById.get(params.sessionId);
    if (!session)
        throw new Error('Quiz session not found');
    if (session.userId !== params.userId)
    ...
    if (session.completed) {
        ...
        if (existingEntry) {
            ...
            const masteryMap = (0, analyticsStore_1.getUserMastery)(session.userId);
    ...
    const updated = (0, analyticsStore_1.updateTopicMastery)(params.userId, session.topic, delta);
    const historyEntry = {
        id: session.id,
...
const getQuizHistory = (userId) => (historyByUser.get(userId) || []).slice().reverse();
