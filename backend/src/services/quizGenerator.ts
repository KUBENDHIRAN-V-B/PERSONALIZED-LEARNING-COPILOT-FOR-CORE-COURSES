...
export interface QuizGenerationRequest {
  ...
  count: number;
  apiKeys: Array<{ key: string; provider: ProviderType }>;
 ...
 */
function validateInputs(request: QuizGenerationRequest): { valid: boolean; error?: string } {
 ...
 */
function constructPrompt(request: QuizGenerationRequest): string {
 ...
 */
function validateQuizResponse(response: any): { valid: boolean; questions?: QuizQuestion[]; error?: string } {
  try {
...
export async function generateQuizQuestions(
  ...
  userId: string
): Promise<QuizGenerationResponse | QuizGenerationError> {
...
Generate exactly ${neededQuestions} questions in the array.`;
        ...
        try {
          const followUpResponse = await callAIProvider(
            {
